...
void generateDirName(char *name,int isboot)
{
	struct tm *ptime;
	time_t the_time;
	char command[256] = {0};
	...
	sprintf(name,"%s/ubtlog.%03d.%02d%02d%02d_%02d%02d%02d",SYSTEM_PATH,
			last_ubtlog_recordid+1,ptime->tm_year % 100, ptime->tm_mon + 1,
			ptime->tm_mday, ptime->tm_hour,
			ptime->tm_min, ptime->tm_sec);
	if(isboot)
		strcat(name,".BOOT");
	mkdir(name, DIR_PERMIT);
	last_ubtlog_recordid++;
	sprintf(command, "echo %d > %s",last_ubtlog_recordid,last_ubtlog_file);
	system(command);
}
...
void generateFileName(char *name)
{
	...
	sprintf(name,"log_%02d%02d%02d",
			ptime->tm_hour,
			ptime->tm_min, ptime->tm_sec);
}
void generatefullName(char *fullname,char *outputDirName,char *outputFileName){
	sprintf(fullname,"%s/%s",outputDirName,outputFileName);
}
...
int is_LogDiskFull()
{
    ...
    statfs(SYSTEM_PATH,&diskInfo);
    unsigned long long totalBlocks = diskInfo.f_bsize;
    unsigned long long freeDisk = diskInfo.f_bfree*totalBlocks;
	//printf("diskInfo.f_bsize=%ld,f_bfree=%ld,f_blocks=%ld\n",diskInfo.f_bsize,diskInfo.f_bfree,diskInfo.f_blocks);
    return freeDisk < 100*1024*1024;
}

unsigned long long getFileSysSize(void)
{
	DIR* dir;
    struct dirent *entry;
    struct stat info;
    long long total_size = 0;
    char* stack[100]; // 栈，用于存储文件夹路径
    int top = -1; // 栈顶指针
    
    stack[++top] = strdup(SYSTEM_PATH); // 将初始文件夹路径入栈
    
    while (top >= 0) {
        char* current_path = stack[top--]; // 出栈
        ...
        while ((entry = readdir(dir)) != NULL) {
            ...
            if (stat(file_path, &info) == 0) {
                if (S_ISREG(info.st_mode)) { // 判断是否为普通文件
                ...
                strcmp(entry->d_name, "..") != 0) 
                {
                    stack[++top] = strdup(file_path); // 子文件夹路径入栈
                }
            } else {
                perror("stat");
            }
...
int is_LogDirFull()
{
    return getFileSysSize() > 300*1024*1024;
}
...
android_logcat_context create_android_logcat() {
    ...
    context->envs.clear();
    context->logRotateSizeKBytes = 4096;
...
static void rotateLogs(android_logcat_context_internal* context) {
	//printf("context->outputFileName:%s\n",context->outputFileName);
    // Can't rotate logs if we're not outputting to a file
    if (!context->outputFileName[0]) return;

    close_output(context);

    if(have_rotates+1 >= context->maxRotatedLogs){
		generateDirName(context->outputDirName,0);
		generateFileName(context->outputFileName);
		have_rotates = 0;
    }else{
    	generateFileName(context->outputFileName);
		have_rotates++;
	...
	char fullname[512] = {0};
	generatefullName(fullname,context->outputDirName,context->outputFileName);
	printf("[%s] fullname:%s\n",__func__, fullname);

    context->output_fd = openLogFile(fullname);
...
static void processBuffer(android_logcat_context_internal* context,
    ...
    if (dev->binary) {
        ...
        err = android_log_processBinaryLogBuffer(
                &buf->entry, &entry, context->eventTagMap, binaryMsgBuf,
                sizeof(binaryMsgBuf));
         printf(">>> pri=%d len=%lu msg='%s'\n",
    ...
    if (android_log_shouldPrintLine(
        ...
        if (match || context->printItAnyways) {
            // FILE * fp = fdopen(context->output_fd, "web");
            // if(NULL == fp){
            //     printf("[%s] %d : fp is null\n",__func__, __LINE__);
            //     exit(-1);
            // }
            bytesWritten = android_log_printLogLine(context->logformat, context->output, &entry);
    ...
    if (context->logRotateSizeKBytes > 0 &&
        ...
        while(is_LogDiskFull() || is_LogDirFull()) {
            printf("current full & clr working directory: %s\n",getcwd(NULL,0));
            system("rm -rf `ls -1t |grep ubtlog |tail -n 1`");
        }
        rotateLogs(context);
    }
}
...
#define QEMU_CMDLINE "qemu.cmdline"
	...
	generateDirName(context->outputDirName,1);
	generateFileName(context->outputFileName);
    ...
    if (!!context->logRotateSizeKBytes && !context->outputFileName[0]) {
        logcat_panic(context, HELP_TRUE, "-r requires -f as well\n");
        goto exit;
    }

    if (!!setId) {
        if (!context->outputFileName[0]) {
            logcat_panic(context, HELP_TRUE,
